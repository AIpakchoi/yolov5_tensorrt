...
ICudaEngine* createEngine(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt) {
    ...
    // 19
    auto conv19 = convBnLeaky(network, weightMap, *bottleneck_csp17->getOutput(0), 256, 3, 2, 1, "model.19");
    ...
    // 21
    auto bottleneck_csp21 = bottleneckCSP(network, weightMap, *cat20->getOutput(0), 512, 512, 3, false, 1, 0.5, "model.21");
    ...
    // 23
    auto conv23 = convBnLeaky(network, weightMap, *bottleneck_csp21->getOutput(0), 512, 3, 2, 1, "model.23");
    ...
    // 25
    auto bottleneck_csp25 = bottleneckCSP(network, weightMap, *cat24->getOutput(0), 1024, 1024, 3, false, 1, 0.5, "model.25");
...
int main(int argc, char** argv) {
    ...
    if (argc == 2 && std::string(argv[1]) == "-s") {
        ...
        std::cerr << "./yolov5l -s  // serialize model to plan file" << std::endl;
        std::cerr << "./yolov5l -d ../samples  // deserialize plan file and run inference" << std::endl;
        return -1;
    }
