...
ICudaEngine* createEngine(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt) {
    ...
    // 19
    auto conv19 = convBnLeaky(network, weightMap, *bottleneck_csp17->getOutput(0), 320, 3, 2, 1, "model.19");
    ...
    // 21
    auto bottleneck_csp21 = bottleneckCSP(network, weightMap, *cat20->getOutput(0), 640, 640, 4, false, 1, 0.5, "model.21");
    ...
    // 23
    auto conv23 = convBnLeaky(network, weightMap, *bottleneck_csp21->getOutput(0), 640, 3, 2, 1, "model.23");
    ...
    // 25
    auto bottleneck_csp25 = bottleneckCSP(network, weightMap, *cat24->getOutput(0), 1280, 1280, 4, false, 1, 0.5, "model.25");
...
int main(int argc, char** argv) {
    ...
    if (argc == 2 && std::string(argv[1]) == "-s") {
        ...
        std::ofstream p("yolov5x.engine", std::ios::binary);
        if (!p) {
        ...
        std::ifstream file("yolov5x.engine", std::ios::binary);
        if (file.good()) {
        ...
        std::cerr << "./yolov5x -s  // serialize model to plan file" << std::endl;
        std::cerr << "./yolov5x -d ../samples  // deserialize plan file and run inference" << std::endl;
        return -1;
    }
    ...
    for (int f = 0; f < (int)file_names.size(); f++) {
        ...
        /*
        auto end = std::chrono::system_clock::now();
        std::cout << "preprocess + inference + nms time:" << std::endl;
        std::cout << std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count() << "ms" << std::endl;
        */
